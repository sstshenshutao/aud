...
//			}
    ...
    private void connect(B_TreeNode prev, B_TreeNode succ, int number) {
    	for(int i=0;i<number;i++) {
			prev.setKey(number+1+i, succ.getKey(i));
			prev.setC(number+1+i, succ.getC(i));
		}
		prev.setC(2*number+1, succ.getC(number));
	}

	private int getIndexNL(int i, B_TreeNode x, String key) {
    	int ki=i;
...
//		right.setC(right.getN()-1, right.getC(right.getN()));
	...
	private void leftRotate(int ci3, B_TreeNode x) {
		...
		//right:
		right.getAllKey().add(0, y);
		right.getAllc().add(0, left.getC(left.getN()));
//		right.setC(right.getN()+1, right.getC(right.getN()));
